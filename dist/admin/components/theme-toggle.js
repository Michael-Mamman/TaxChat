...
const ThemeToggle = () => {
    ...
    const rootPath = useSelector((state) => state.paths?.rootPath ?? "/admin");
    ...
    return (_jsx(Box, { as: "button", type: "button", onClick: onClick, title: label, "aria-label": label, disabled: busy, style: {
            display: "flex",
            alignItems: "center",
            ...
            opacity: busy ? 0.5 : 1,
        }, children: _jsx(Icon, { icon: isDark ? "Sun" : "Moon", size: 18 }) }));
};
