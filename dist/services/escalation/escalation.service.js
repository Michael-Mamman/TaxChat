...
class EscalationService {
    ...
    async forwardToAgent(phone, message) {
        ...
        if (!context?.escalation_ticket_id) {
            ...
            await ConversationContext.findOneAndUpdate({ phone }, { $set: { is_escalated: false }, $unset: { escalated_to: 1, escalation_ticket_id: 1 } });
            await whatsappService.sendMessage(phone, "Sorry - I lost the connection to that officer. Type *MENU* to see what I can help with, or *AGENT* to try again.");
        ...
        await whatsappService.sendMessage(phone, "Passed to the officer handling your case. Type *MENU* to return to self-service.");
        console.log('[escalation.service::forwardToAgent] EXIT', { forwarded: true });
    }
    async returnFromEscalation(phone, resolution, endedBy = "officer") {
