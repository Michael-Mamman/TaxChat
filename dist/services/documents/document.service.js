 ...
 */
function authorisation(doc, reference) {
    const y = 500;
    doc
        .moveTo(50, y)
        .lineTo(250, y)
...
async function footer(doc, reference) {
    ...
    doc
        ...
        .fontSize(9)
        .font("Helvetica")
        .text(`Scan the code, or enter reference ${reference} at ${VERIFY_BASE_URL}.`, 146, y + 38, { width: 399 });
    doc
        .fillColor(COLOURS.muted)
        .fontSize(8)
...
function render(filePath, build) {
    ...
    async generateTCC(input) {
        ...
        await render(filePath, async (doc) => {
            ...
            doc
                ...
                .fontSize(10)
                .font("Helvetica")
                .text("This is to certify that the taxpayer named above has met their tax obligations " +
                "for the period assessed, and that no tax liability remains outstanding as at the date of issue.", { width: 495, align: "left" });
    ...
    async generateTINCertificate(input) {
        ...
        await render(filePath, async (doc) => {
            ...
            if (input.registeredOn)
                field(doc, "Registered on", input.registeredOn);
            doc.moveDown(0.6);
            doc
                .fillColor(COLOURS.ink)
                .fontSize(10)
                .font("Helvetica")
                .text("This certificate confirms the registration of the taxpayer named above. " +
                "It shows your Tax Identification Number in full - keep it secure and share it only where required.", { width: 495 });
            authorisation(doc, input.reference);
            await footer(doc, input.reference);
        });
        ...
        await render(filePath, async (doc) => {
            ...
            for (const a of input.assessments) {
                ...
                doc
                    ...
                    .fontSize(10)
                    .font("Helvetica")
                    .text(`Assessed ${money(a.assessed_amount)}   Paid ${money(a.paid_amount)}   ` +
                    `Balance ${money(a.balance)}`);
                doc.text(`Status: ${a.status}${a.due_date ? `   Due: ${a.due_date}` : ""}`);
                doc.moveDown(0.6);
            }
            const totals = input.assessments.reduce((acc, a) => ({
            ...
            doc
                ...
                .fontSize(10)
                .font("Helvetica")
                .text(`Assessed ${money(totals.assessed)}   Paid ${money(totals.paid)}   ` +
