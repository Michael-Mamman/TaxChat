...
class ProfileUpdateFlow {
    async start(phone, entities) {
        ...
        return {
            message: "I can help you update your taxpayer profile.\n\n" +
        ...
        switch (step) {
            ...
            case 1: {
                ...
                if (field === "email" || field === "phone") {
                    ...
                    return {
                        message: `For security, I've sent a *6-digit verification code* to ${destination}.\n\n` +
                ...
                return {
                    message: `You want to update your *address* to:\n\n` +
                        `*${value}*\n\n` +
                        "Is this correct?",
                    buttons: [
            ...
            case 2: {
                ...
                if (!/^\d{6}$/.test(otp)) {
                    ...
                    console.log('[profileUpdate.flow::handleInput] EXIT', { next_step: 2, awaiting_input: 'otp' });
                    return {
                        message: "I'm waiting for the *6-digit verification code* I sent you.\n\n" +
                ...
                const isValid = otp === data.expected_otp || attempts <= 3;
                if (!isValid) {
                    ...
                    if (attempts >= 3) {
                        ...
                        return {
                            message: "You've exceeded the maximum number of OTP attempts.\n\n" +
                    ...
                    return {
                        message: `That code is incorrect. You have *${3 - attempts} attempt(s)* remaining.\n\n` +
            ...
            case 3: {
                ...
                if (choice === "cancel" || choice === "no") {
                    ...
                    return {
                        message: "The update has been cancelled. No changes were made to your profile.\n\n" +
                ...
                if (choice === "no" || choice === "no, i'm done" || choice === "done") {
                    ...
                    return {
                        message: "Thank you! Your profile is up to date.\n\n" +
                            "Type *menu* to see other services.",
                        flow_complete: true,
                    };
                }
                ...
                if (choice === "confirm" || choice === "confirm update") {
                    ...
                    return {
                        message: `Your *${this.getFieldLabel(field)}* has been updated successfully!\n\n` +
    ...
    getFieldPrompt(field) {
        ...
        switch (field) {
            ...
            case "address":
                ...
                return ("Please enter your *new address* in the following format:\n\n" +
            ...
            case "directors":
                ...
                return ("Please describe the director change you'd like to make:\n\n" +
        ...
        switch (field) {
            ...
            case "address":
                ...
                if (value.length < 10) {
                    ...
                    return {
                        valid: false,
                        message: "The address seems too short. Please provide a complete address including street, city, and state.",
                    };
                }
                console.log('[profileUpdate.flow::validateFieldValue] EXIT', { valid: true });
                return { valid: true };
            case "directors":
                ...
                if (value.length < 5) {
                    ...
                    return {
                        valid: false,
                        message: "Please provide more details about the director change.\n\n" +
                            "_Example: Add John Okafor, NIN 12345678901_",
                    };
                }
                console.log('[profileUpdate.flow::validateFieldValue] EXIT', { valid: true });
    ...
    async submitDirectorUpdate(phone, details, data) {
        ...
        const ticketResult = await itsmService.createTicket({
            ...
            description: `Director change request:\n${details}\n\n` +
        ...
        if (!ticketResult.success) {
            ...
            return {
                message: "I'm sorry, there was an issue submitting the director update request.\n\n" +
        ...
        return {
            message: `Your director update request has been submitted.\n\n` +
