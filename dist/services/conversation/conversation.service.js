...
function isEscalationRequest(normalizedText) {
    if (ESCALATION_KEYWORDS.includes(normalizedText))
        return true;
...
function isGreeting(normalizedText) {
    return GREETING_KEYWORDS.some((kw) => normalizedText === kw || normalizedText.startsWith(kw + " "));
}
class ConversationService {
    ...
    async handleInteractiveResponse(phone, response, convo) {
        console.log("[conversation.service::handleInteractiveResponse] ENTER", { phone: phone ? `${phone.slice(0, 4)}***` : null, selectedId: response?.id, currentFlow: convo?.current_flow });
        // Handle list reply
        ...
        await whatsappService.sendMessage(phone, "That option isn't available any more - it belongs to a request we've already finished.");
        await menuService.sendMainMenu(phone);
