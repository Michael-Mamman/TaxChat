...
export const decryptRequest = (body, privatePem, passphrase) => {
    ...
    const formattedKey = privatePem
        ...
        .join("\n")
        .trim();
    const formattedPassphrase = passphrase?.trim().replace(/\"/g, "");
    let privateKey;
    try {
        privateKey = crypto.createPrivateKey({
            key: formattedKey,
            passphrase: formattedPassphrase,
        });
    }
    catch (err) {
        console.error("[FlowEncryption] Failed to create private key:", err.message);
        throw err;
    }
