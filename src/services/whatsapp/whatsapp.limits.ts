 ...
 */
export function splitInteractiveBody(body: string): { preamble?: string; body: string } {
  const t = body.trim();
  if (t.length <= WA.INTERACTIVE_BODY_MAX) return { body: t };
...
export function toRow(o: { id: string; title: string; description?: string }): {
  id: string;
  title: string;
  description?: string;
} {
  ...
  if (o.description) row.description = clamp(o.description, WA.ROW_DESCRIPTION_MAX);
  return row;
}
