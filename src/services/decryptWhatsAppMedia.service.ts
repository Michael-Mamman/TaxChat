...
import type { FlowDocument } from "../types/flow.types.js";
import type { FlowDataExchangePayload } from "../types/flow.types.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export async function decryptWhatsAppMedia(document: FlowDocument): Promise<{
  filePath: string;
  base64: string;
}> {
  ...
  const hmac = crypto
    ...
    .update(
      Buffer.concat([Buffer.from(iv, "base64"), ciphertext]),
    )
    .digest();
...
export async function handleFlowDataExchange(
  payload: FlowDataExchangePayload,
) {
