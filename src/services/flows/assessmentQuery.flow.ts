...
class AssessmentQueryFlow {
  ...
  async handleInput(
    ...
    switch (step) {
      ...
      case 3: {
        ...
        const disputeOptions = data.dispute_options as Array<{
          assessment_id: string;
          reference?: string;
          tax_type: string;
          period?: string;
  ...
  private async fetchAssessments(
    ...
    const summary = assessments.map((a) => {
      const statusLabel =
        ...
        a.status;
      return (
        `- *${a.tax_type}* (${a.period ?? `Year ${a.tax_year}`})\n` +
  ...
  private async fetchOutstandingLiabilities(
    phone: string,
    tin: string,
    data: Record<string, unknown>,
  ): Promise<FlowStepResult> {
    ...
    const summary = liabilities.map((l) => {
      return (
        `- *${l.tax_type}* (${l.period ?? `Year ${l.tax_year}`})\n` +
