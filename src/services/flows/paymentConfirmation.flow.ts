...
class PaymentConfirmationFlow {
    ...
    switch (step) {
      ...
      case 0: {
        ...
        if (!/^\d{10,20}$/.test(digits)) {
          ...
          return {
            message:
              "That doesn't look like a payment reference.\n\n" +
  ...
  private handlePostLookupAction(
    phone: string,
    input: string,
    data: Record<string, unknown>,
  ): FlowStepResult {
    ...
    if (choice === "trace" || choice === "trace payment") {
      ...
      return {
        message:
          "I'll create a payment trace request. This will be investigated by our finance team.\n\n" +
  ...
  private async createPaymentTrace(
    phone: string,
    data: Record<string, unknown>,
  ): Promise<FlowStepResult> {
