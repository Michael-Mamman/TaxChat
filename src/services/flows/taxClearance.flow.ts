    ...
    switch (step) {
      ...
      case 1: {
        ...
        if (data.is_compliant) {
          ...
          const reference = (data.tcc_reference as string) ?? `TCC-${randomReference()}`;
          ...
          const certificate = needsCertificate
            ? await documentService.generateTCC({
                reference,
                taxpayerName: (data.taxpayer_name as string) ?? "Taxpayer",
          ...
          if (choice === "download" || choice === "download tcc") {
            ...
            return {
              message:
                "Your Tax Clearance Certificate is attached.\n\n" +
