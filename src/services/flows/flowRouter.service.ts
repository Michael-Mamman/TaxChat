  ...
  private async processFlowResult(
    ...
    if (result.document) {
      ...
      try {
        ...
        await whatsappService.sendMessage(
          phone,
          "I couldn't attach your document just now. Quote the reference above at your tax office, or type *AGENT* and an officer will send it.",
        );
      } finally {
  ...
  private async handleAuthInput(
    ...
    if (awaiting === "tin") {
      ...
      if (!/^\d{11}$/.test(identifier)) {
        ...
        await whatsappService.sendMessage(
          phone,
          "A NIN and a BVN are both *11-digit numbers*. Please check and re-enter.\n\n" +
      ...
      if (!/^\d{11}$/.test(kycNin)) {
        ...
        await whatsappService.sendMessage(
          phone,
          "A NIN is an *11-digit number*. Please check and re-enter it.\n\n" +
            "_Type *MENU* to start over, or *AGENT* to speak with an officer._",
        );
