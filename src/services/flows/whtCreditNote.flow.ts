...
class WHTCreditNoteFlow {
      ...
      case 4: {
        ...
        if (trimmed.length >= 10) {
          const ticketResult = await itsmService.createTicket({
            ...
            description:
              `Phone: ${phone}\n` +
              `Agent TIN: ${(data.agent_tin as string) ?? "N/A"}\n` +
              `Payment Reference: ${(data.payment_reference as string) ?? "N/A"}\n\n` +
              `Issue Description:\n${trimmed}`,
            phone,
            priority: "medium",
          });
  ...
  private async lookupWHTDeduction(
    phone: string,
    data: Record<string, unknown>,
  ): Promise<FlowStepResult> {
    const lookupMethod = data.lookup_method as string;
  ...
  private parseCreditDetails(
    ...
    const [period, tin] = lines as [string, string];
  ...
  private async submitCreditNoteRequest(
    phone: string,
    data: Record<string, unknown>,
  ): Promise<FlowStepResult> {
    const wht = data.selected_wht as {
